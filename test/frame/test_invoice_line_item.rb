...
class TestInvoiceLineItem < Minitest::Test
  ...
  def test_retrieve_invoice_line_item
    ...
    stub_api_request(
      :get,
      "/v1/invoice_line_items/#{line_item_id}",
      "invoice_line_item.json"
    )

    line_item = Frame::InvoiceLineItem.retrieve(line_item_id)
  ...
  def test_update_invoice_line_item
    ...
    assert_requested :patch, "#{Frame.api_base}/v1/invoice_line_items/#{line_item_id}", times: 1
  end
  ...
  def test_list_invoice_line_items
    ...
    line_items = Frame::InvoiceLineItem.list
  ...
  def test_list_invoice_line_items_with_params
    ...
    line_items = Frame::InvoiceLineItem.list(page: 1, per_page: 20)

    assert_equal 2, line_items.data.size
    assert_requested :get, "#{Frame.api_base}/v1/invoice_line_items",
  ...
  def test_delete_invoice_line_item
    ...
    deleted_line_item = Frame::InvoiceLineItem.delete(line_item_id)
    ...
    assert_requested :delete, "#{Frame.api_base}/v1/invoice_line_items/#{line_item_id}", times: 1
  end
end
